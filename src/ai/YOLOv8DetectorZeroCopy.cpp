...
#endif
{
    LOG_INFO() << "[YOLOv8DetectorZeroCopy] Creating zero-copy optimized detector";
}
...
YOLOv8DetectorZeroCopy::~YOLOv8DetectorZeroCopy() {
    ...
    cleanupZeroCopy();
#endif
    LOG_INFO() << "[YOLOv8DetectorZeroCopy] Zero-copy detector destroyed";
    g_perfStats.print();
}

bool YOLOv8DetectorZeroCopy::initialize(const std::string& modelPath, InferenceBackend backend) {
    LOG_INFO() << "[YOLOv8DetectorZeroCopy] Initializing zero-copy detector...";
    
    // 首先调用基类初始化
    if (!YOLOv8Detector::initialize(modelPath, backend)) {
        LOG_ERROR() << "[YOLOv8DetectorZeroCopy] Base initialization failed";
        return false;
    }
    ...
    if (backend == InferenceBackend::RKNN) {
        ...
        if (!initializeZeroCopy()) {
            LOG_ERROR() << "[YOLOv8DetectorZeroCopy] Zero-copy initialization failed";
            return false;
        }
        LOG_INFO() << "[YOLOv8DetectorZeroCopy] Zero-copy mode enabled successfully";
#else
        LOG_INFO() << "[YOLOv8DetectorZeroCopy] RKNN not available, using standard mode";
#endif
    }
...
bool YOLOv8DetectorZeroCopy::initializeZeroCopy() {
    if (m_rknnContext == 0) {
        LOG_ERROR() << "[YOLOv8DetectorZeroCopy] RKNN context not initialized";
        return false;
    }
    ...
    if (ret < 0) {
        LOG_ERROR() << "[YOLOv8DetectorZeroCopy] Failed to query I/O number: " << ret;
        return false;
    }
    
    LOG_INFO() << "[YOLOv8DetectorZeroCopy] Model has " << m_ioNum.n_input 
              << " inputs, " << m_ioNum.n_output << " outputs";
    ...
    for (uint32_t i = 0; i < m_ioNum.n_input; i++) {
        ...
        if (ret < 0) {
            LOG_ERROR() << "[YOLOv8DetectorZeroCopy] Failed to query input attr " << i;
            return false;
        }
    }
    ...
    for (uint32_t i = 0; i < m_ioNum.n_output; i++) {
        ...
        if (ret < 0) {
            LOG_ERROR() << "[YOLOv8DetectorZeroCopy] Failed to query output attr " << i;
            return false;
        }
    }
    ...
    if (m_inputMem == nullptr) {
        LOG_ERROR() << "[YOLOv8DetectorZeroCopy] Failed to create input memory";
        return false;
    }
    ...
    if (m_outputMem == nullptr) {
        LOG_ERROR() << "[YOLOv8DetectorZeroCopy] Failed to create output memory";
        return false;
    }
    
    LOG_INFO() << "[YOLOv8DetectorZeroCopy] Created DMA buffers - Input: " 
              << m_inputAttrs[0].size << " bytes, Output: " 
              << m_outputAttrs[0].size << " bytes";
...
bool YOLOv8DetectorZeroCopy::preprocessToBuffer(const cv::Mat& frame, void* buffer) {
    ...
    if (m_inputAttrs[0].type == RKNN_TENSOR_UINT8) {
        ...
        LOG_ERROR() << "[YOLOv8DetectorZeroCopy] Unsupported input type";
        return false;
    }
...
std::vector<YOLOv8DetectorZeroCopy::Detection> YOLOv8DetectorZeroCopy::inferenceZeroCopy(const cv::Mat& frame) {
    ...
    if (ret < 0) {
        LOG_ERROR() << "[YOLOv8DetectorZeroCopy] Failed to set inputs: " << ret;
        return detections;
    }
    ...
    if (ret < 0) {
        LOG_ERROR() << "[YOLOv8DetectorZeroCopy] Failed to run inference: " << ret;
        return detections;
    }
    ...
    if (ret < 0) {
        LOG_ERROR() << "[YOLOv8DetectorZeroCopy] Failed to get outputs: " << ret;
        return detections;
    }
    ...
    LOG_INFO() << "[ZeroCopy] Frame processed in " << total_time << "ms "
              << "(prep: " << preprocess_time << "ms, inf: " << inference_time 
              << "ms, post: " << postprocess_time << "ms)";
