...
bool YOLOv8Detector::initializeRKNN(const std::string& modelPath) {
    ...
    if (ret < 0) {
        std::cout << "[YOLOv8Detector] Warning: Failed to set multi-core NPU, using default core: " << ret << std::endl;
    } else {
        std::cout << "[YOLOv8Detector] Successfully enabled 3-core NPU acceleration" << std::endl;
    }
    ...
    if (io_num.n_input > 0) {
        ...
        if (input_attrs[0].n_dims == 4) {
            ...
            std::cout << "[YOLOv8Detector] RKNN model input size: " << m_inputWidth << "x" << m_inputHeight << "x" << channels << std::endl;
            std::cout << "[YOLOv8Detector] Input format: " << (input_attrs[0].fmt == RKNN_TENSOR_NHWC ? "NHWC" : "NCHW") << std::endl;
            std::cout << "[YOLOv8Detector] Input type: " << get_type_string(input_attrs[0].type) << std::endl;
        }
    }
...
bool YOLOv8Detector::initializeOpenCV(const std::string& modelPath) {
    ...
    std::cout << "[YOLOv8Detector] OpenCV DNN is disabled, falling back to simulation" << std::endl;
...
std::vector<YOLOv8Detector::Detection> YOLOv8Detector::detectWithRKNN(const cv::Mat& frame) {
    ...
    std::cout << "[YOLOv8Detector] Input tensor size: " << inputs[0].size
              << " bytes, type: " << get_type_string(inputs[0].type)
              << ", format: " << get_format_string(inputs[0].fmt) << std::endl;
    ...
    for (uint32_t i = 0; i < io_num.n_output; i++) {
        ...
        if (ret < 0) {
            std::cerr << "[YOLOv8Detector] Failed to query output attr " << i << ": " << ret << std::endl;
            return detections;
        }
    }
...
cv::Mat YOLOv8Detector::preprocessImageForRKNN(const cv::Mat& image) {
    ...
    cv::Mat result;
    std::cout << "[YOLOv8Detector] Input attrs type: " << get_type_string(m_rknnInputAttrs.type) << std::endl;

    if (m_rknnInputAttrs.type == RKNN_TENSOR_FLOAT32) {
        // Convert to float32 and normalize to [0, 1]
        resized.convertTo(result, CV_32F, 1.0/255.0);
        std::cout << "[YOLOv8Detector] Preprocessed to FLOAT32 with normalization, size: "
                  << result.total() * result.elemSize() << " bytes" << std::endl;
        ...
        resized.convertTo(result, CV_32F, 1.0/255.0);
        std::cout << "[YOLOv8Detector] Preprocessed to FLOAT32 for FP16 model with normalization, size: "
                  << result.total() * result.elemSize() << " bytes" << std::endl;
    } else {
        // Keep as uint8 for quantized models (INT8, UINT8, etc.)
        resized.convertTo(result, CV_8U);
        std::cout << "[YOLOv8Detector] Preprocessed to UINT8 without normalization, size: "
                  << result.total() * result.elemSize() << " bytes" << std::endl;
    }
...
std::vector<YOLOv8Detector::Detection> YOLOv8Detector::postprocessRKNNResultsOfficial(rknn_output* outputs, rknn_tensor_attr* output_attrs, uint32_t n_output, const cv::Size& originalSize) {
    ...
    if (n_output == 1) {
        ...
        std::cout << "[YOLOv8Detector] Processing single-output YOLOv8 model" << std::endl;

        // Print output dimensions for debugging
        std::cout << "[YOLOv8Detector] Output dims: ";
        for (uint32_t i = 0; i < output_attrs[0].n_dims; i++) {
            std::cout << output_attrs[0].dims[i];
            if (i < output_attrs[0].n_dims - 1) std::cout << "x";
        }
        std::cout << std::endl;
        ...
        if (output_attrs[0].type == RKNN_TENSOR_INT8) {
            ...
            std::cout << "[YOLOv8Detector] Converting quantized output to float" << std::endl;
            ...
            std::cout << "[YOLOv8Detector] Output type: " << get_type_string(output_attrs[0].type) << std::endl;
            std::cout << "[YOLOv8Detector] Output buffer size: " << outputs[0].size << " bytes" << std::endl;

            // Safety check
            if (outputs[0].buf == nullptr) {
                std::cerr << "[YOLOv8Detector] Error: Output buffer is null!" << std::endl;
                return detections;
            }
            ...
            std::cout << "[YOLOv8Detector] Expected output size: " << expected_size << " bytes" << std::endl;

            if (outputs[0].size < expected_size) {
                std::cerr << "[YOLOv8Detector] Error: Output buffer too small!" << std::endl;
                return detections;
            }
            ...
            if (output_attrs[0].type == RKNN_TENSOR_FLOAT16) {
                ...
                std::cout << "[YOLOv8Detector] Converted FP16 to FP32 using proper IEEE 754 conversion" << std::endl;
    ...
    std::cout << "[YOLOv8Detector] RKNN post-processing: " << validCount << " raw detections -> ";
    ...
    std::cout << detections.size() << " final detections" << std::endl;
    return detections;
}
