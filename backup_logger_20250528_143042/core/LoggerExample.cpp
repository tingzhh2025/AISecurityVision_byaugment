 ...
 * @brief Logger使用示例
 * 
 * 演示如何使用Logger类替代std::cout和std::cerr
 */
...
void demonstrateBasicLogging() {
    ...
    // 替代std::cout和std::cerr的用法
    LOGGER_OUT << "替代std::cout的输出";
    LOGGER_ERR << "替代std::cerr的错误输出";
}
...
// 替换原有std::cout和std::cerr的示例函数
void oldStyleLogging() {
    // 原来的代码风格
    // std::cout << "[Info] 这是信息日志" << std::endl;
    // std::cerr << "[Error] 这是错误日志" << std::endl;
...
2. 替换std::cout和std::cerr：
   LOGGER_OUT << "替代std::cout";
   LOGGER_ERR << "替代std::cerr";
